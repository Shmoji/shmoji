import { Animator, Behaviour, serializeable, getWorldQuaternion, setWorldQuaternion, setWorldPosition, getWorldPosition } from "@needle-tools/engine";
...
export class IFrameContentShmoji extends Behaviour {
    ...
    start() {
        ...
        // need "enablejsapi=1" so that this code can play iframe video: "iframe.contentWindow?.postMessage('{"event":"command","func":"playVideo","args":""}', '*')" 
        this.iframe.src =  this.url + '?enablejsapi=1';
        ...
        this.iframe.width = scaleX + 'px';
        this.iframe.height = scaleY + 'px';
    ...
    playVideo() {
        ...
        if (this.iframe) {
            console.log('iframe exists and postMessage called')
